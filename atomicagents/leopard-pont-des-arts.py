...
orchestrator_agent.register_context_provider(
...
def execute_tool(
    ...
    orchestrator_output: OrchestratorOutputSchema,
) -> Union[TavilySearchToolOutputSchema, CalculatorToolOutputSchema]:
    if orchestrator_output.tool == "search":
        return tavily_tool.run(orchestrator_output.tool_parameters)
...
if __name__ == "__main__":
    ...
    tavily_tool = TavilySearchTool(
        config=TavilySearchToolConfig(api_key=os.getenv("TVLY_KEY"), max_results=5)
    )
    ...
    console.print(
        Panel(
            orchestrator_agent.system_prompt_generator.generate_prompt(),
            title="System Prompt",
            expand=False,
        )
    )
    ...
    for user_input in inputs:
        ...
        response = execute_tool(tavily_tool, calculator_tool, orchestrator_output)
